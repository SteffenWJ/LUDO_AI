...
class Qplayer:
    def __init__(self, tableName="QLearn/Qs.txt", newtbl=False, isTrain=False, weights:dict=None):
    ...
    def reward(self, max_Q_new, piece):
        def zero(self, weights):
            # Open
            r = weights.get("Open", 0.25)
            return r

        def one(self, weights):
            # Normal move
            r = weights.get("Normal", 0.0001)
            return r

        def two(self, weights):
            # Goal move
            r = weights.get("Goal", 0.5)
            return r

        def three(self, weights):
            # Star Move
            r = weights.get("Star", 0.6)
            return r

        def four(self, weights):
            # Globe Move
            r = weights.get("Globe", 0.4)
            return r

        def five(self, weights):
            # Protect move
            r = weights.get("Protect", 0.2)
            return r

        def six(self, weights):
            # Kill move
            r = weights.get("Kill", 0.9)
            return r

        def seven(self, weights):
            # Backwards goal zone move
            r = weights.get("Reverse_goal_zone", 0.0)
            return r

        def eight(self, weights):
            # Goal Zone Move
            r = weights.get("Goal_zone", 0.4)
            return r
        ...
        r = func(self, weights=self.weights)
        if (self.state % 3) == 0:
            r += 0.1
        delta_LUT = LR * (r + DELTA * max_Q_new - self.LUT[self.state, self.action])
